...
import { VisualSquadEditor } from "../../../components/visual-squad-editor";
...
const euro2028FinalGroups: Group[] = [
  { title: "Gardiens", short: "G", players: [p("Arijanet Murić",29,"Ipswich",64,0),p("Amir Saipi",27,"Lugano",9,0),p("Mustafë Abdullahu",24,"Lokomotiva",1,0)] },
  { title: "Défenseurs", short: "D", players: [p("Ron Raçi",25,"Hajduk",7,0),p("Amir Rrahmani",34,"Parthenope",91,8),p("Albian Hajdari",24,"TSG Hoffenheim",21,1),p("Leard Sadriu",27,"Campionii FC Argeș",20,0),p("Arnis Ademi",21,"Fiorentina",2,0),p("Florent Hadergjonaj",33,"Alanyaspor",61,1),p("Drešević",31,"FC Machida",35,1),p("Art Smakaj",25,"LASK Linz",4,0),p("Mërgim Vojvoda",33,"Reims",89,4),p("Lumbardh Dellova",29,"CSKA Sofia",19,3)] },
  { title: "Milieux et ailiers", short: "M", players: [p("Luan Simnica",24,"Almere City",0,0),p("Elvis Rexhbeçaj",30,"Valencia",34,3),p("Leon Avdullahu",24,"FC Bayern München",27,1),p("Bledian Krasniqi",26,"Zürich",20,0),p("Lirim Kastrati",29,"Malines",19,2),p("Emir Sahiti",29,"Eintracht Braunschweig",13,3),p("Bersant Celina",31,"AIK",56,4),p("Edon Zhegrova",29,"AS Monaco",67,16),p("Florent Muslija",29,"F.C. Hansa Rostock",57,6),p("Milot Rashica",31,"Beşiktaş",85,15),p("Fisnik Asllani",25,"Liverpool",28,10)] },
  { title: "Attaquants", short: "A", players: [p("Vedat Muriqi",34,"Mallorca",79,38),p("Albion Rrahmani",27,"Sparta Prague",20,6)] }
];
const euro2028PrelistGroups: Group[] = [
  { title: "Gardiens", short: "G", players: [p("Arijanet Murić",29,"Ipswich",64,0),p("Amir Saipi",27,"Lugano",9,0),p("Visar Bekaj",30,"Oviedo",10,0),p("Mustafë Abdullahu",24,"Lokomotiva",1,0)] },
  { title: "Défenseurs", short: "D", players: [p("Andi Hoti",25,"Eintracht Braunschweig",6,0),p("Ron Raçi",25,"Hajduk",7,0),p("Amir Rrahmani",34,"Parthenope",91,8),p("Albian Hajdari",24,"TSG Hoffenheim",21,1),p("Leard Sadriu",27,"Campionii FC Argeș",20,0),p("Arnis Ademi",21,"Fiorentina",2,0),p("Florent Hadergjonaj",33,"Alanyaspor",61,1),p("Drešević",31,"FC Machida",35,1),p("Art Smakaj",25,"LASK Linz",4,0),p("Mërgim Vojvoda",33,"Reims",89,4),p("Dion Gallapeni",23,"Widzew",6,0)] },
  { title: "Milieux et ailiers", short: "M", players: [p("Luan Simnica",24,"Almere City",0,0),p("Elvis Rexhbeçaj",30,"Valencia",34,3),p("Leon Avdullahu",24,"FC Bayern München",27,1),p("Hamza Muqaj",22,"Holstein Kiel",7,1),p("Bledian Krasniqi",26,"Zürich",20,0),p("Veldin Hodža",25,"Rubin",14,0),p("Emir Sahiti",29,"Eintracht Braunschweig",13,3),p("Lirim Kastrati",29,"KV Mechelen",19,2),p("Bersant Celina",31,"AIK",56,4),p("Edon Zhegrova",29,"AS Monaco",67,16),p("Florent Muslija",29,"F.C. Hansa Rostock",57,6),p("Milot Rashica",31,"Beşiktaş",85,15),p("Fisnik Asllani",25,"Liverpool",28,10)] },
  { title: "Attaquants", short: "A", players: [p("Vedat Muriqi",34,"Mallorca",79,38),p("Albion Rrahmani",27,"Sparta Prague",20,6)] }
];
...
const legacyBreaks = ["Liste définitive Euro 2028", "Pré-liste Euro 2028", "Mars 2028", "Novembre 2027", "Septembre / Octobre 2027", "Juin 2027", "Mars 2027", "Novembre 2026", "Septembre / Octobre 2026"];
const breakDescriptions: Record<string, string> = {
  "Liste définitive Euro 2028": "26 joueurs · Allemagne, Angleterre et République tchèque · Groupe B",
  "Pré-liste Euro 2028": "30 joueurs · Écosse (D), Pays de Galles (E) et préparation du Championnat d’Europe",
...
export default function ConvocationsPage() {
  const [breakDate, setBreakDate] = useState(legacyBreaks[0]);
  const [remoteSquads,setRemoteSquads]=useState<any[]>([]);
  const [hidden,setHidden]=useState<Set<string>>(new Set());
  useEffect(()=>{const db=getSupabaseClient();if(!db)return;Promise.all([db.from("squads").select("*").order("squad_date",{ascending:false}),db.from("hidden_content").select("content_key").eq("content_type","squad")]).then(([saved,removed])=>{const blocked=new Set((removed.data||[]).map(x=>x.content_key));setHidden(blocked);const visible=(saved.data||[]).filter(x=>!blocked.has(x.title));setRemoteSquads(visible);if(visible.length)setBreakDate(visible[0].title)})},[]);
  const remote=remoteSquads.find(s=>s.title===breakDate);
  const remoteGroups:Group[]=remote?[{title:"Joueurs convoqués",short:"S",players:(remote.players||[]).map((line:string)=>{const [name,age="0",club="À renseigner",caps="0",goals="0"]=line.split("|").map(x=>x.trim());return p(name,Number(age),club,Number(caps),Number(goals))})}]:[];
  const groups = remote?remoteGroups:breakDate === "Liste définitive Euro 2028" ? euro2028FinalGroups : breakDate === "Pré-liste Euro 2028" ? euro2028PrelistGroups : breakDate === "Mars 2028" ? march2028Groups : breakDate === "Novembre 2027" ? november2027Groups : breakDate === "Septembre / Octobre 2027" ? autumn2027Groups : breakDate === "Juin 2027" ? juneGroups : breakDate === "Mars 2027" ? marchGroups : breakDate === "Novembre 2026" ? novemberGroups : septemberGroups;
  const breaks=[...remoteSquads.map(s=>s.title),...legacyBreaks.filter(b=>!remoteSquads.some(s=>s.title===b)&&!hidden.has(b))];
  const isCurrent = true;
  const total = groups.reduce((sum, group) => sum + group.players.length, 0);
  const editableSquad=remote||{title:breakDate,description:breakDescriptions[breakDate],players:groups.flatMap(group=>group.players.map(player=>`${player.name} | ${player.age} | ${player.club} | ${player.caps} | ${player.goals}`))};
  return <main className="min-h-screen bg-[radial-gradient(circle_at_85%_0,rgba(124,58,237,.18),transparent_30%),#090b10] px-5 py-7 text-slate-100 lg:px-12"><div className="mx-auto max-w-5xl"><SelectionHeader active="Convocations"/><section className="surface mt-6 overflow-hidden"><div className="flex flex-col justify-between gap-4 border-b border-white/[.08] p-6 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-300">Kosovo</p><h2 className="mt-2 text-2xl font-semibold">Joueurs sélectionnés</h2><p className="mt-1 text-sm text-slate-400">Liste des convocations par trêve internationale.</p></div><div className="flex flex-wrap items-center gap-2"><VisualSquadEditor/><VisualSquadEditor squad={editableSquad}/><label className="rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-sm"><span className="mr-2 text-slate-400">Trêve</span><select value={breakDate} onChange={e=>setBreakDate(e.target.value)} className="bg-transparent font-semibold outline-none">{breaks.map(x=><option key={x} className="bg-panel">{x}</option>)}</select></label></div></div>{isCurrent ? <><div className="flex items-center gap-3 border-b border-white/[.08] bg-violet-400/[.05] px-6 py-4 text-sm"><span className="grid h-7 w-7 place-items-center rounded-full bg-violet-400/15 text-violet-200"><Check size={15}/></span><p><strong>{total} joueurs convoqués</strong><span className="text-slate-400"> · {remote?.competition||breakDescriptions[breakDate]}</span></p></div><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-white/[.08] text-xs uppercase tracking-[.12em] text-slate-500"><tr><th className="px-6 py-4 font-semibold">Joueur</th><th className="px-4 py-4 font-semibold">Poste</th><th className="px-4 py-4 font-semibold">Âge</th><th className="px-4 py-4 font-semibold">Club</th><th className="px-4 py-4 text-center font-semibold">Sélections</th><th className="px-6 py-4 text-center font-semibold">Buts</th></tr></thead><tbody>{groups.map(group=><GroupRows key={group.title} group={group}/>)}</tbody></table></div></> : <div className="grid place-items-center py-16 text-center"><Users size={32} className="text-violet-300"/><h3 className="mt-4 text-lg font-semibold">Aucune convocation importée</h3><p className="mt-2 max-w-sm text-sm leading-6 text-slate-400">La sélection de {breakDate} apparaîtra ici dès que les données de la carrière internationale seront ajoutées.</p></div>}</section></div></main>;
}
