...
import { VisualMatchEditor } from "../../../components/visual-match-editor";
...
const calendars: Record<"2026" | "2027" | "2028", CalendarYear> = {
  ...
  "2027": {
    ...
    matches: [
      ...
      { date: "Lun. 29 mars 2027 · 19:30", opponent: "Slovénie", venue: "E", score: "3 - 0", competition: "Qualifications Euro 2028", slug: "slovenie-kosovo-2027" },
      { date: "Jeu. 10 juin 2027 · 19:30", opponent: "Danemark", venue: "D", score: "1 - 0", competition: "Qualifications Euro 2028", slug: "kosovo-danemark-2027" },
      { date: "Dim. 13 juin 2027 · 19:30", opponent: "Lituanie", venue: "E", score: "1 - 2", competition: "Qualifications Euro 2028", slug: "lituanie-kosovo-2027" },
  ...
  "2028": {
    ...
    matches: [
      ...
      { date: "Sam. 10 juin 2028 · 15:00", opponent: "Allemagne", venue: "D", score: "0 - 3", competition: "Euro 2028 · Groupe B", note: "Hampden Park · Terrain neutre", slug: "kosovo-allemagne-euro-2028" },
      { date: "Mer. 14 juin 2028 · 14:00", opponent: "Angleterre", venue: "E", score: "3 - 1", competition: "Euro 2028 · Groupe B", note: "Wembley · Terrain neutre", slug: "angleterre-kosovo-euro-2028" },
      { date: "Lun. 19 juin 2028 · 20:00", opponent: "République tchèque", venue: "D", score: "1 - 2", competition: "Euro 2028 · Groupe B", note: "Hampden Park · Terrain neutre", slug: "republique-tcheque-kosovo-euro-2028" },
      { date: "Ven. 22 sept. 2028 · 19:30", opponent: "Turquie", venue: "D", score: null, competition: "Ligue des Nations B · Groupe 4" },
      { date: "Lun. 25 sept. 2028 · 19:30", opponent: "Croatie", venue: "E", score: null, competition: "Ligue des Nations B · Groupe 4" },
      { date: "Ven. 29 sept. 2028 · 19:30", opponent: "Autriche", venue: "E", score: null, competition: "Ligue des Nations B · Groupe 4" },
      { date: "Lun. 2 oct. 2028 · 19:30", opponent: "Croatie", venue: "D", score: null, competition: "Ligue des Nations B · Groupe 4" },
      { date: "Sam. 18 nov. 2028 · 19:30", opponent: "Turquie", venue: "E", score: null, competition: "Ligue des Nations B · Groupe 4" },
      { date: "Mar. 21 nov. 2028 · 19:30", opponent: "Autriche", venue: "D", score: null, competition: "Ligue des Nations B · Groupe 4" },
    ],
  },
};
...
export default function SelectionCalendarPage() {
  ...
  const [remoteMatches, setRemoteMatches] = useState<CalendarMatch[]>([]);
  const [hidden,setHidden]=useState<Set<string>>(new Set());
  useEffect(()=>{const db=getSupabaseClient();if(!db)return;Promise.all([db.from("matches").select("*").order("match_date"),db.from("hidden_content").select("content_key").eq("content_type","match")]).then(([matches,removed])=>{setHidden(new Set((removed.data||[]).map(x=>x.content_key)));if(!matches.data)return;setRemoteMatches(matches.data.map(row=>{const d=new Date(row.match_date),home=row.home_team==="Kosovo";return {date:new Intl.DateTimeFormat("fr-FR",{weekday:"short",day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}).format(d),opponent:home?row.away_team:row.home_team,venue:home?"D":"E",score:row.home_score==null?null:`${row.home_score} - ${row.away_score}`,competition:row.competition,slug:row.slug,note:row.venue}}))})},[]);
  const legacy = calendars[year];
  const currentRemote = remoteMatches.filter(match=>match.date.includes(year));
  const remoteSlugs = new Set(currentRemote.map(match=>match.slug));
  const calendar = {...legacy,matches:[...currentRemote.filter(match=>!match.slug||!hidden.has(match.slug)),...legacy.matches.filter(match=>(!match.slug||!remoteSlugs.has(match.slug))&&(!match.slug||!hidden.has(match.slug)))]};

  return <main className="min-h-screen bg-[radial-gradient(circle_at_85%_0,rgba(124,58,237,.18),transparent_30%),#090b10] px-5 py-7 text-slate-100 lg:px-12"><div className="mx-auto max-w-5xl"><SelectionHeader active="Calendrier"/><section className="surface mt-6 overflow-hidden"><header className="flex flex-col justify-between gap-4 border-b border-white/[.07] p-6 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-300">{calendar.eyebrow}</p><h2 className="mt-2 text-xl font-semibold">Calendrier du Kosovo · {year}</h2><p className="mt-1 text-sm text-slate-400">{calendar.description}</p></div><div className="flex items-center gap-2"><VisualMatchEditor/><label className="rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-sm"><span className="mr-2 text-slate-400">Année</span><select value={year} onChange={event => setYear(event.target.value as keyof typeof calendars)} className="bg-transparent font-semibold outline-none"><option className="bg-panel" value="2026">2026</option><option className="bg-panel" value="2027">2027</option><option className="bg-panel" value="2028">2028</option></select></label></div></header>{calendar.matches.length > 0 ? <div className="divide-y divide-white/[.07]">{calendar.matches.map((match, index) => <MatchRow key={`${match.date}-${match.opponent}`} match={match} highlighted={index === 0}/>)}</div> : <div className="grid place-items-center px-6 py-16 text-center"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-500/15 text-violet-300"><CalendarDays size={24}/></span><h3 className="mt-4 text-lg font-semibold">Qualifications Euro 2028 · Groupe J</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-400">Le Kosovo est engagé dans le groupe J. Les rencontres seront affichées ici dès l’import des adversaires et des dates.</p></div>}</section><p className="mt-4 flex items-center gap-2 text-xs text-slate-500"><CalendarDays size={14}/>D = domicile · E = extérieur · Cliquez sur un match terminé pour ouvrir le compte-rendu.</p></div></main>;
}
...
function MatchRow({ match, highlighted }: { match: CalendarMatch; highlighted: boolean }) {
  ...
  return <article className="flex items-center"><div className="min-w-0 flex-1">{match.slug?<a href={`/selection/calendrier/${match.slug}`} className={className}>{content}<ChevronRight className="text-violet-300 sm:hidden" size={17}/></a>:<div className={className}>{content}</div>}</div><div className="pr-3"><VisualMatchEditor match={match}/></div></article>;
}
